...
function renderTypeSelector() {
    ...
    container.innerHTML = state.types.map(t => `
        <button onclick="selectType('${t.name}', this)"
                class="type-btn py-2.5 rounded-xl text-xs font-bold transition-all ${t.name === state.selectedType ? 'bg-indigo-600 text-white shadow-sm' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}">
            ${t.name}
        </button>
    `).join('');
}
...
function renderRecords() {
    ...
    if (display.length === 0) {
        list.innerHTML = `
            <div class="text-center py-12">
                ...
                <p class="text-sm text-slate-400 font-medium">暂无记录</p>
                <p class="text-xs text-slate-300 mt-1">开始运动吧！</p>
            </div>`;
        return;
    }
    ...
    // Each exercise type has a distinct color for quick visual scanning (like Strava)
    const typeColors = {
        '跑步': '#6366f1',     // indigo
        '游泳': '#3b82f6',     // blue
        '壶铃': '#f59e0b',     // amber
    ...
    list.innerHTML = display.map(r => {
        ...
        const accent = typeColors[r.exercise_type] || fallback;
        return `
            <div class="mb-3 bg-white rounded-2xl overflow-hidden shadow-sm border border-slate-50 hover:shadow-md transition-shadow">
                <div class="flex">
                    ...
                    <div class="flex-1 p-4 flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-2 flex-shrink-0 ml-2">
                            <div class="text-right">
                                <div class="text-sm font-bold text-slate-800 whitespace-nowrap">${r.duration_minutes} min</div>
                                <div class="text-xs text-slate-400 whitespace-nowrap">${r.quantity} ${unit}</div>
                            </div>
                            ${isOwn || state.currentUser?.role === 'admin' ? `
                                <button onclick="deleteRecord(${r.id})" class="p-1.5 text-slate-300 hover:text-red-400 hover:bg-red-50 rounded-lg transition-colors">
    ...
    if (sorted.length === 0) {
        list.innerHTML = `
            <div class="text-center py-12">
                ...
                <p class="text-sm text-slate-400 font-medium">本周暂无数据</p>
            </div>`;
        return;
    }

    const userColors = ['#6366f1', '#f59e0b', '#10b981', '#ec4899'];

    list.innerHTML = `
        <h3 class="text-sm font-bold text-slate-400 mb-3 px-1">📊 成员运动统计</h3>
    ` + sorted.map((s, idx) => {
        const uColor = userColors[idx % userColors.length];
        const unitMap = {};
        state.types.forEach(t => { unitMap[t.name] = t.unit; });
            ...
            <div class="bg-white rounded-2xl overflow-hidden shadow-sm border border-slate-50 mb-3">
                <div class="flex items-center gap-4 p-5 pb-3">
                    ...
                    <div class="text-right flex-shrink-0">
                        ...
                        <div class="font-bold text-lg text-slate-800">${s.total_duration} <span class="text-xs text-slate-400 font-normal">min</span></div>
                    </div>
                </div>
...
function setViewMode(mode) {
    ...
    ['all', 'me', 'stats'].forEach(m => {
        ...
        if (m === mode) {
            btn.className = 'flex-1 py-2.5 rounded-xl text-xs font-bold shadow-sm bg-slate-900 text-white transition-all';
        } else {
            btn.className = 'flex-1 py-2.5 rounded-xl text-xs font-bold text-slate-400 transition-all';
        }
    });
    loadRecords();
}
...
function renderInviteList() {
    ...
    if (state.invites.length === 0) {
        list.innerHTML = '<p class="text-xs text-slate-400 text-center py-2">暂无邀请记录</p>';
        return;
    }
    list.innerHTML = '<p class="text-xs text-slate-400 mb-2">历史邀请</p>' + state.invites.slice(0, 5).map(inv => {
        const status = inv.is_used ? '✅ 已用' : '⏳ 有效';
        const color = inv.is_used ? 'text-slate-400' : 'text-green-600';
        return `
            <div class="flex justify-between items-center py-2 border-b border-slate-50 text-xs">
                <span class="text-slate-600 font-mono">${(inv.token || '').slice(0, 8)}...</span>
