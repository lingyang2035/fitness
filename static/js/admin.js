...
function switchTab(tab) {
    ...
    ['dashboard', 'users', 'records', 'logs', 'invites'].forEach(t => {
        ...
        if (t === tab) {
            btn.className = 'flex-1 py-2 rounded-xl text-xs font-semibold bg-orange-600 text-white transition-all';
        } else {
    ...
    area.innerHTML = `
        <div class="grid grid-cols-2 gap-3 mb-4">
            <div class="bg-orange-50 p-4 rounded-2xl">
                <div class="text-xs text-orange-400 mb-1">用户总数</div>
                <div class="text-2xl font-bold text-orange-700">${data.total_users}</div>
            </div>
...
async function renderUsers(area) {
    ...
    area.innerHTML = `
        <div class="flex justify-between items-center mb-4">
            ...
            <button onclick="openUserModal()" class="bg-orange-600 text-white px-4 py-2 rounded-xl text-xs font-bold hover:bg-orange-700 transition-colors">+ 添加用户</button>
        </div>
        <div class="space-y-2">
            ${users.map(u => `
                <div class="flex items-center justify-between p-3 bg-slate-50 rounded-xl">
                    <div class="flex items-center gap-3">
                        <div class="w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm ${u.role === 'admin' ? 'bg-amber-100 text-amber-600' : 'bg-orange-100 text-orange-600'}">
                            ${u.display_name[0]}
                        </div>
                        <div>
                    ...
                    <div class="flex gap-1">
                        <button onclick="openUserModal(${u.id})" class="p-2 text-slate-400 hover:text-orange-600">
    ...
    area.innerHTML = `
        ...
        <div class="space-y-2 max-h-[500px] overflow-y-auto">
            ${records.length === 0 ? '<p class="text-center text-slate-400 py-4 text-sm">暂无记录</p>' : records.map(r => `
                <div class="flex items-center justify-between p-3 bg-slate-50 rounded-xl">
                    <div class="flex items-center gap-3">
                        <div class="w-8 h-8 bg-orange-100 text-orange-600 rounded-lg flex items-center justify-center font-bold text-xs">
...
async function renderInvites(area) {
    ...
    area.innerHTML = `
        <div class="flex justify-between items-center mb-3">
            ...
            <button onclick="generateAdminInvite()" class="bg-orange-600 text-white px-4 py-2 rounded-xl text-xs font-bold hover:bg-orange-700 transition-colors">+ 生成新邀请</button>
        </div>
        <div id="admin-invite-url-box" class="hidden bg-orange-50 p-4 rounded-xl mb-4">
            <p class="text-xs text-orange-400 mb-1">新邀请链接</p>
            <p class="text-sm font-mono text-orange-700 break-all" id="admin-invite-url-text"></p>
            <button onclick="copyAdminInviteUrl()" class="mt-2 w-full bg-orange-600 text-white py-2 rounded-xl font-bold text-xs hover:bg-orange-700 transition-colors">复制链接</button>
        </div>
        <div class="space-y-2">
